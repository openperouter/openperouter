...
var _ = Describe("Sample YAML Validation", func() {
	...
	// Dynamically collect and test all YAML files in the samples directory
	samplesDir := "."
	sampleFiles, err := testutils.DiscoverFiles(samplesDir, []string{".yaml", ".yml"})
	Expect(err).NotTo(HaveOccurred())
	for _, sampleFile := range sampleFiles {
		// Get the relative path from the samples directory
		...
		Context(relPath, func() {
			It("should successfully create resources from "+relPath, func() {
				Expect(testutils.ValidateResourceFromFile(k8sClient, relPath, namespace)).To(Succeed())
			})
		})
	}
})
