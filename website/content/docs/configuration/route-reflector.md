...
In OpenPERouter this is useful to distribute EVPN (type-2/type-3) and VTEP reachability routes between nodes without relying on the Top-of-Rack fabric to reflect them, or to build a hierarchy where one or more nodes act as spines for the cluster.
...
2. **`bgp listen limit 65535`** whenever a neighbor uses `listenRange`, raising FRR's dynamic-neighbor cap.
...
- The default `clusterID` (`192.0.2.1`) is an RFC 5737 documentation address, guaranteed not to overlap a real router-id pool.
