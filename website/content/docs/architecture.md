...
OpenPERouter consists of three main components deployed as DaemonSets, ensuring one instance runs on each Kubernetes node:
...
- **Host BGP Sessions**: Establishes BGP sessions with components running on the host
...
1. **Interface Migration**: Moves the host interface connected to the external router into the router's pod network namespace
2. **Network Interface Creation**: For each VNI, creates the required network interfaces (bridge, VRF, VXLAN) for FRR operation
3. **Veth Pair Setup**: Creates veth pairs to connect VRFs to the host, assigns IPs from the `localCIDR`, and moves one end to the router's namespace
4. **Configuration Deployment**: Generates the FRR configuration and sends it to the router pod for application

### Node Labeller

The node labeller is a critical component that ensures consistent resource allocation across the cluster.

#### Purpose and Responsibilities

The node labeller provides persistent node indexing and is responsible for:
...
The node labeller persists the assigned index as a Kubernetes node label, making it available to other components. This ensures:
