...
func SetupPassthrough(ctx context.Context, client *Client, params hostnetwork.PassthroughParams) error {
	...
	if err := hostnetwork.AssignIPsToInterface(hostTap, params.CrossNsIPs.HostIPv4, params.CrossNsIPs.HostIPv6); err != nil {
		return fmt.Errorf("failed to assign IPs to host TAP: %w", err)
	}

	// Assign IPs to the grout port "pt-ns" via grcli.
	if err := ensurePortAddresses(ctx, client, portName, params.CrossNsIPs.NSIPv4, params.CrossNsIPs.NSIPv6); err != nil {
