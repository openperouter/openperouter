...
func TestPeerASN(t *testing.T) {
	...
	for _, tc := range tcs {
		peerASN, err := NewPeerASN(tc.number, tc.asnType)
		if err != nil {
		...
		if peerASN.Equal(mustNewPeerASNFromNumber(tc.referenceASN)) != tc.expectedEqual {
			t.Fatalf("%s: Equal(): %t does not match %t",
				tc.name, peerASN.Equal(mustNewPeerASNFromNumber(tc.referenceASN)), tc.expectedEqual)
		}
	}
}
