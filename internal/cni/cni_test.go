...
func TestAddInvokesPluginOnceAndCaches(t *testing.T) {
	...
	invoked, err := Add(context.Background(), params)
	if err != nil {
		t.Fatalf("first Add failed: %v", err)
	}
	if !invoked {
		t.Fatal("first Add should invoke the plugin")
	}
	...
	if invoked {
		t.Fatal("second Add should be served from the cache")
	}
...
func TestAddWithSinglePluginConf(t *testing.T) {
	...
	if !invoked {
		t.Fatal("Add should invoke the plugin")
	}
}
...
func TestAddWithInvalidConfig(t *testing.T) {
	...
	if _, err := Add(context.Background(), params); err == nil {
...
func TestAddForwardsDeclaredCapabilitiesOnly(t *testing.T) {
	...
	params := env.params("net1", map[string]interface{}{
	...
	runtimeConfig, hasRuntimeConfig := stdin["runtimeConfig"].(map[string]interface{})
	if !hasRuntimeConfig {
...
func TestDelCachedTearsDownAndClearsCache(t *testing.T) {
	env := newFakePluginEnv(t)

	if _, err := Add(context.Background(), env.params("net1", nil)); err != nil {
		t.Fatalf("Add failed: %v", err)
	}

	if err := DelCached(context.Background(), env.binDirs(), env.cacheDir, containerID); err != nil {
	...
	if err := DelCached(context.Background(), env.binDirs(), env.cacheDir, containerID); err != nil {
		t.Fatalf("second DelCached failed: %v", err)
	}
	if commands := env.loggedCommands(t); len(commands) != 2 {
		t.Fatalf("second DelCached should not invoke the plugin, got %v", commands)
	}
}

func TestDelCachedKeepsRequestedInterfaces(t *testing.T) {
	env := newFakePluginEnv(t)

	for _, ifName := range []string{"net1", "net2"} {
		if _, err := Add(context.Background(), env.params(ifName, nil)); err != nil {
			t.Fatalf("Add %s failed: %v", ifName, err)
		}
	}

	if err := DelCached(context.Background(), env.binDirs(), env.cacheDir, containerID, "net1"); err != nil {
		t.Fatalf("DelCached failed: %v", err)
	}
	...
	invoked, err := Add(context.Background(), env.params("net1", nil))
	if err != nil {
		t.Fatalf("Add after DelCached failed: %v", err)
	}
	if invoked {
		t.Fatal("Add for the kept interface should be served from the cache")
	}
}

func TestDelCachedWithEmptyCache(t *testing.T) {
	env := newFakePluginEnv(t)

	if err := DelCached(context.Background(), env.binDirs(), env.cacheDir, containerID); err != nil {
		t.Fatalf("DelCached on empty cache failed: %v", err)
	}
	if commands := env.loggedCommands(t); len(commands) != 0 {
		t.Fatalf("DelCached on empty cache should not invoke the plugin, got %v", commands)
	}
}
...
func TestValidateConfig(t *testing.T) {
	tests := []struct {
		...
		{name: "malformed json", config: `{"cniVersion": `, wantErr: true},
		{name: "conflist with broken plugins", config: `{"cniVersion": "1.0.0", "name": "u", "plugins": "notalist"}`, wantErr: true},
	}
...
const containerID = "perouter-underlay-test"
...
func newFakePluginEnv(t *testing.T) *fakePluginEnv {
	...
	// ADD, emits a canned CNI result. Environment variables are inherited
...
func (e *fakePluginEnv) params(ifName string, capabilityArgs map[string]interface{}) Params {
	return Params{
...
func (e *fakePluginEnv) binDirs() []string {
...
func (e *fakePluginEnv) pluginStdin(t *testing.T, command, ifName string) map[string]interface{} {
	t.Helper()
