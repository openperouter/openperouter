...
func TestValidateCNIDeviceImmutable(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			oldUnderlay: underlayWithCNI("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`),
			newUnderlay: underlayWithCNI("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`),
		},
		{
			name:        "changed rawConfig is rejected",
			oldUnderlay: underlayWithCNI("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`),
			newUnderlay: underlayWithCNI("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"ipvlan"}]}`),
			errorString: "rawConfig for interface \"net1\" is immutable",
		},
		{
			name:        "new cni interface name passes",
			oldUnderlay: underlayWithCNI("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`),
			newUnderlay: underlayWithCNI("net2", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"ipvlan"}]}`),
		},
		{
		...
		},
		{
			...
			oldUnderlay: underlayWithCNIDevice("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`, `{"mac":"02:00:00:00:00:01"}`),
			newUnderlay: underlayWithCNIDevice("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`, `{"mac":"02:00:00:00:00:01"}`),
		},
		{
			name:        "changed runtimeConfig is rejected",
			oldUnderlay: underlayWithCNIDevice("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`, `{"mac":"02:00:00:00:00:01"}`),
			newUnderlay: underlayWithCNIDevice("net1", `{"cniVersion":"1.0.0","name":"u","plugins":[{"type":"macvlan"}]}`, `{"mac":"02:00:00:00:00:02"}`),
