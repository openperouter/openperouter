...
var _ = Describe("Underlay CNI configuration", func() {
	...
	var testDir string
	var logPath string
	var cniRuntime CNIRuntime

	cniParams := func(ifNames ...string) UnderlayParams {
		interfaces := make([]CNIInterfaceParams, 0, len(ifNames))
		for _, ifName := range ifNames {
			interfaces = append(interfaces, CNIInterfaceParams{
	...
	BeforeEach(func() {
		...
		cniRuntime = CNIRuntime{
			...
			ContainerID: CNIContainerIDForNode("testnode"),
		}
	})
	...
	It("provisions a cni interface marked as underlay", func() {
	...
	It("works combined with a network device", func() {
		Expect(createInterface(underlayCNITestInterface, underlayCNITestPhysIP)).To(Succeed())
		params := cniParams("net1")
		params.UnderlayInterfaces = []string{underlayCNITestInterface}
		...
		err := netnamespace.In(testNs, func() error {
			link, err := netlink.LinkByName(underlayCNITestInterface)
			Expect(err).NotTo(HaveOccurred())
			Expect(link.Attrs().Group).To(Equal(uint32(underlayGroupID)))
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
	})

	It("requires a namespace rebuild when a cni interface is removed from the spec", func() {
		Expect(SetupUnderlay(context.Background(), cniParams("net1"))).To(Succeed())

		err := SetupUnderlay(context.Background(), cniParams("underlay0"))
		Expect(err).To(BeAssignableToTypeOf(UnderlayExistsError("")))
	})

	It("removes the cni interfaces and their cache on RemoveCNIUnderlay", func() {
		Expect(SetupUnderlay(context.Background(), cniParams("net1"))).To(Succeed())

		Expect(RemoveCNIUnderlay(context.Background(), cniRuntime)).To(Succeed())
		Expect(loggedCommands()).To(ConsistOf("ADD net1", "DEL net1"))
		err := netnamespace.In(testNs, func() error {
			_, err := netlink.LinkByName("net1")
			Expect(err).To(HaveOccurred(), "the cni interface should be gone")
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
	...
	It("provisions again after the namespace is rebuilt and the cache cleared", func() {
		...
		// Simulate the non-recoverable error flow: CNI DEL, then netns
		// rebuild, then a reconcile with a different interface name.
		Expect(RemoveCNIUnderlay(context.Background(), cniRuntime)).To(Succeed())
		cleanTest(underlayCNITestNS)
		testNs = createTestNS(underlayCNITestNS)
...
// in the namespace, carries the underlay group ID, is up and has the address
// the fake plugin assigns.
func validateCNIInterfaceInNS(ns netns.NsHandle, ifName string) {
	err := netnamespace.In(ns, func() error {
		...
		validateGroupID(Default, link, underlayGroupID)
