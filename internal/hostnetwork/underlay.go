...
type UnderlayInterface struct {
	// InterfaceName tesll the nic name inside the router pod.
...
func SetupUnderlay(ctx context.Context, params UnderlayParams) error {
	...
	for _, iface := range params.UnderlayInterfaces {
		if err := SetupUnderlayInterface(ctx, targetNetNS, params, iface); err != nil {
			return fmt.Errorf("failed setting up underlay interfaces: %w", err)
		}
	}
...
// SetupUnderlayInterface provisions a single underlay interface according to
// its kind: host network devices are moved into the namespace, CNI
// interfaces are added by invoking their plugin. Both operations are
// idempotent.
func SetupUnderlayInterface(ctx context.Context, ns netns.NsHandle, params UnderlayParams,
	iface UnderlayInterface) error {
	ifName := iface.InterfaceName
	switch iface.Kind {
	case UnderlayInterfaceNetDev:
		if err := moveInterfaceFromDefaultNetns(ctx, ns, ifName); err != nil {
			return fmt.Errorf("failed to setup underlay net device %s: %w", ifName, err)
		}
	case UnderlayInterfaceCNIDev:
		if err := cni.GetInvoker().Add(ctx, cni.AddParams{
			Config:         iface.CNI.Config,
			NetNS:          params.TargetNS,
			IfName:         ifName,
			CapabilityArgs: iface.CNI.CapabilityArgs,
		}); err != nil {
			return fmt.Errorf("failed to setup underlay cni device %s: %w", ifName, err)
		}
	default:
		return fmt.Errorf("underlay interface %s has unsupported kind %q", ifName, iface.Kind)
	}
	return nil
}
...
func underlayInterfaces(ns netns.NsHandle) ([]UnderlayInterface, error) {
	...
	if cni.GetInvoker() == nil {
		return res, nil
	}
	cniIfaces, err := cni.GetInvoker().CachedIfNames()
	if err != nil {
...
// are not requested anymore, preserving how they were provisioned.
func UnderlayInterfacesToRemove(existing,
	...
	for _, iface := range requested {
		requestedNames[iface.InterfaceName] = struct{}{}
	}
	removed := []UnderlayInterface{}
	for _, iface := range existing {
		if _, found := requestedNames[iface.InterfaceName]; !found {
...
func RestoreUnderlay(ctx context.Context, fromNetNSPath string, ifacesToRemove []UnderlayInterface) error {
	...
	toRemoveByName := make(map[string]UnderlayInterface, len(ifacesToRemove))
	for _, iface := range ifacesToRemove {
		toRemoveByName[iface.InterfaceName] = iface
	}
	restoreUnderlay := func(ctx context.Context, fromNetNSHandle, defaultNetNSHandle *netlink.Handle,
		...
		var errs []error
		for _, l := range links {
			ifaceToRemove, found := toRemoveByName[l.Attrs().Name]
			if !found {
				continue
			}
			switch ifaceToRemove.Kind {
			case UnderlayInterfaceCNIDev:
				if err := cni.GetInvoker().Del(ctx, l.Attrs().Name); err != nil {
					return fmt.Errorf("failed to delete cni underlay interfaces %v: %w", l.Attrs().Name, err)
				}
			case UnderlayInterfaceNetDev:
