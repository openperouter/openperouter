...
func EnsureBridge(ctx context.Context, ovs libovsclient.Client, bridgeName string) (string, error) {
	br := &vswitchd.Bridge{Name: bridgeName}
	err := ovs.Get(ctx, br)
	if err == nil {
		return br.UUID, nil
	}
	...
	mutateOp, err := ovs.WhereCache(func(*vswitchd.OpenvSwitch) bool { return true }).
...
func ensurePortAttachedToBridge(ctx context.Context, ovs libovsclient.Client, bridgeUUID, interfaceName string) error {
	iface := &vswitchd.Interface{Name: interfaceName}
	interfaceUUID := ""
	...
	port := &vswitchd.Port{Name: interfaceName}
	portUUID := ""
	...
	bridge := &vswitchd.Bridge{UUID: bridgeUUID}
...
func ensureInternalPortForBridge(ctx context.Context, ovs libovsclient.Client, bridgeUUID, bridgeName string) error {
	...
	iface := &vswitchd.Interface{Name: bridgeName}
	err := ovs.Get(ctx, iface)
	if err == nil {
