...
// TestRequireRoot skips the current Ginkgo test if CAP_SYS_ADMIN is not available.
func TestRequireRoot() {
	if !HasCapSysAdmin() {
		ginkgo.Skip(skipPrivileged)
	}
}

// TestRequireRootT skips the current standard Go test if CAP_SYS_ADMIN is not available.
func TestRequireRootT(t *testing.T) {
	t.Helper()
	if !HasCapSysAdmin() {
