...
func vtepIPForConfig(evpn *v1alpha1.EVPNConfig, targetNS string, nodeIndex int) (string, error) {
	if evpn.VTEPCIDR != "" {
		...
		return vtepIPFromInterface(targetNS, evpn.VTEPInterface)
	}

	return "", fmt.Errorf("missing vtep CIDR or Interface a Underlay config")
}

func vtepIPFromInterface(targetNS, vtepInterface string) (string, error) {
	addrs, err := hostnetwork.InterfaceIPsForNamespace(targetNS, vtepInterface)
	if err != nil {
		return "", fmt.Errorf("failed to get IPs for vtep interface %q on network namespace %q: %w", vtepInterface, targetNS, err)
	}

	if len(addrs) == 0 {
		return "", fmt.Errorf("missing addresses to configure vtep")
	}
	...
	if selectedIPNet == nil {
		return "", fmt.Errorf("missing ipv4 addresse to configure vtep")
	}
