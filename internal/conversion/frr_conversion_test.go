...
func TestAPItoFRRListenRange(t *testing.T) {
			...
			wantNeighbors: []frr.NeighborConfig{
				{
					...
					ID:                    "fd00:10::/64",
					NetworkLayerProtocols: []networklayerprotocol.NLP{ipv6},
				},
			},
			wantListenLimit: 65535,
		},
			...
			wantNeighbors: []frr.NeighborConfig{
				{
					...
					ListenRange:           "192.168.10.0/24",
					ID:                    "192.168.10.0/24",
					NetworkLayerProtocols: []networklayerprotocol.NLP{evpn, ipv4},
					PropertiesByNLP: map[networklayerprotocol.NLP]frr.NLPProperties{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.Underlay.ListenLimit != tt.wantListenLimit {
				t.Errorf("ListenLimit = %d, want %d", got.Underlay.ListenLimit, tt.wantListenLimit)
			}
		})
	}
}
