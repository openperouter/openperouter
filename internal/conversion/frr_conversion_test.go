...
func TestAPItoFRRGracefulRestart(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			gr: &v1alpha1.GracefulRestartConfig{
				RestartTime:   ptr.To(uint32(90)),
				StalePathTime: ptr.To(uint32(180)),
			},
			want: &frr.GracefulRestart{
				RestartTime:   90,
				StalePathTime: 180,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !cmp.Equal(got.Underlay.GracefulRestart, tt.want) {
				t.Errorf("GracefulRestart diff: %s", cmp.Diff(got.Underlay.GracefulRestart, tt.want))
			}
		})
	}
}
