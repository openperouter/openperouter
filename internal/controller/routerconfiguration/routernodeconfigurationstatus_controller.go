...
type RouterNodeConfigurationStatusReconciler struct {
	...
	MyNamespace   string
	Logger        *slog.Logger
	EventReporter *status.StatusManager
}
...
// buildStatus creates the status from EventReporter's shared state
func (r *RouterNodeConfigurationStatusReconciler) buildStatus() v1alpha1.RouterNodeConfigurationStatusStatus {
	// Get aggregated status summary from EventReporter
	statusSummary := r.EventReporter.GetStatusSummary()
...
func (r *RouterNodeConfigurationStatusReconciler) SetupWithManager(mgr ctrl.Manager) error {
	if r.EventReporter == nil {
		return fmt.Errorf("EventReporter is required but not set")
	}
	...
	ctrlBuilder = ctrlBuilder.WatchesRawSource(
		source.Channel(r.EventReporter.GetChannel(), &handler.EnqueueRequestForObject{}),
	)
