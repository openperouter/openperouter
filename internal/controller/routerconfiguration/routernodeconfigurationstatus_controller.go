...
func (r *RouterNodeConfigurationStatusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var status v1alpha1.RouterNodeConfigurationStatus
	...
	if err != nil && !errors.IsNotFound(err) {
		slog.Error("failed to get RouterNodeConfigurationStatus", "error", err)
		return ctrl.Result{}, err
	}

	// If resource already exists, no need to create it again
	if err == nil {
		logger.Info("RouterNodeConfigurationStatus already exists", "name", status.Name)
	...
	if err := r.Create(ctx, routerNodeStatus); err != nil {
		slog.Error("failed to create RouterNodeConfigurationStatus", "error", err)
		return ctrl.Result{}, err
	}
	...
	if err := r.Status().Update(ctx, routerNodeStatus); err != nil {
		slog.Error("failed to update RouterNodeConfigurationStatus status", "error", err)
		return ctrl.Result{}, err
	}
