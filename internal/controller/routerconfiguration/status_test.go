...
func TestBuildStatus(t *testing.T) {
	...
	underlayResult.AddFailure(KindUnderlay, "my-underlay", v1alpha1.ValidationFailed, "bad ASN")

	partialResult := ReconcileResult{}
	partialResult.AddFailure(KindL3VNI, "bad-vni", v1alpha1.ValidationFailed, "invalid vrf")
	partialResult.AddFailure(KindL2VNI, "orphan", v1alpha1.DependencyFailed, "no L3VNI")
	...
	tests := []struct {
		...
		},
		{
			...
			result: ReconcileResult{}.WithError(
				fmt.Errorf("failed to reload frr config: exit status 1"),
			),
			expectedReady:        metav1.ConditionFalse,
			expectedDegraded:     metav1.ConditionTrue,
			expectedReadyReason:  ReasonConfigurationFailed,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := buildStatus(tt.result)

			ready := apimeta.FindStatusCondition(s.Conditions, ConditionTypeReady)
			if ready == nil {
			...
			degraded := apimeta.FindStatusCondition(s.Conditions, ConditionTypeDegraded)
			if degraded == nil {
