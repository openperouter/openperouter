...
func TestValidateNegative(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Neighbor with hostasn equal to asn",
			gvk:  underlayGVK,
			obj: newUnstructured("Underlay", map[string]any{
				"asn": int64(65000),
				"neighbors": []any{
			...
			errSubstr: "hostASN must be different from asn for eBGP",
		},
		{
		...
		{
			name: "L3VNI hostsession with same ASNs",
			gvk:  l3vniGVK,
			obj: newUnstructured("L3VNI", map[string]any{
				"vrf": "testvrf",
				"hostsession": map[string]any{
			...
			errSubstr: "hostASN must be different from asn",
		},
		{
...
func TestValidateMultipleErrors(t *testing.T) {
	t.Run("EVPN with both vteps and Neighbor with equal ASNs", func(t *testing.T) {
		obj := newUnstructured("Underlay", map[string]any{
			"asn": int64(65000),
			"evpn": map[string]any{
				"vtepcidr":      "10.10.0.0/24",
		...
		for _, e := range errs {
			...
			if strings.Contains(msg, "hostASN must be different from asn for eBGP") {
				foundASN = true
			}
		}
		if !foundVtep {
		...
		if !foundASN {
			t.Error("missing ASN validation error")
		}
	})
