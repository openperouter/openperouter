...
The admission webhook and Reconcile loop should ensure that there is no conflict between them for the same node, for example two l2vnis with the same VRF.
...
3. **Multiple not overlapping per node instances allowed**: L3VNI, L2VNI, and L3Passthrough can have multiple instances matching the same node, but they cannot conflict within same node
