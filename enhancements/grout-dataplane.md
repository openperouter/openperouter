...
openperouter:
  grout:
    enabled: false
    image: quay.io/grout/grout:0.15.0
    resources:
      requests:
        memory: 512Mi
        cpu: 250m
...
When grout is enabled, the reconciler FRR as before. After that, the host network
...
- L3VNI scenario implemented with grout.

#### M3
- L2VNI scenario implemented with grout.
- Full e2e test coverage matching the kernel data path test suite.
- Performance benchmarks comparing grout (with SR-IOV support) vs kernel forwarding throughput.

#### M4
- Support for hardware acceleration with SR-IOV NICs, CPU isolation and huge pages
