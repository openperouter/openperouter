...
**Client configuration**: Users manually add the RR node underlay IPs as `type: internal` neighbors in the client Underlay CR (PR #260). The FRR templates already handle iBGP correctly — `allowas-in` is emitted only for eBGP neighbors and `next-hop-self force` is emitted for iBGP neighbors.
...
When control plane nodes participate in the data plane (e.g. `runOnMaster=true`), they get a full Underlay with ToR. Workers include the control plane RR IPs as iBGP neighbors. The control plane Underlay lists all CP node IPs as `type: internal` neighbors for inter-RR peering — each hostcontroller filters out its own IP:
...
#### Resulting frr.conf — CP node as RR + normal router (cp-1)

When CP nodes participate in the data plane, they get the full routing config (ToR eBGP, VTEP, VNI) plus the RR stanzas. The `bgp listen range` entries come from the explicit `listenRanges` in the Underlay CR:
...
router bgp 64514
  ...
  neighbor 192.168.10.2 remote-as 64512          ! eBGP to ToR (IPv4)
  neighbor fd00:10::2 remote-as 64512            ! eBGP to ToR (IPv6)
  neighbor 192.168.10.4 remote-as internal        ! explicit iBGP to other RR (IPv4)
  neighbor fd00:10::4 remote-as internal          ! explicit iBGP to other RR (IPv6)
  ...
  address-family ipv4 unicast
    ...
    neighbor 192.168.10.2 allowas-in             ! eBGP only
  exit-address-family
  ...
  address-family l2vpn evpn
    ...
    neighbor fd00:10::2 allowas-in               ! eBGP only
    neighbor 192.168.10.4 activate               ! other RR, no allowas-in
    neighbor fd00:10::4 activate                 ! other RR, no allowas-in
    advertise-all-vni
    advertise-svi-ip
  exit-address-family
```

#### Resulting frr.conf — CP node as RR only (cp-1)

When CP nodes only do route reflection, there is no ToR eBGP session, no VTEP, and no VNI configuration. The node only reflects routes between iBGP peers:
...
router bgp 64514
  ...
  address-family l2vpn evpn
    neighbor CLIENTS activate
    neighbor CLIENTS route-reflector-client
    neighbor 192.168.10.4 activate               ! other RR
    neighbor fd00:10::4 activate                 ! other RR
  exit-address-family
```

#### Resulting frr.conf — client node (worker-1)
...
router bgp 64514
  ...
  neighbor fd00:11::2 remote-as 64512            ! eBGP to ToR (IPv6)
  neighbor 192.168.10.3 remote-as internal        ! iBGP to RR cp-1 (IPv4)
  neighbor fd00:10::3 remote-as internal          ! iBGP to RR cp-1 (IPv6)
  neighbor 192.168.10.4 remote-as internal        ! iBGP to RR cp-2 (IPv4)
  neighbor fd00:10::4 remote-as internal          ! iBGP to RR cp-2 (IPv6)
  ...
  address-family l2vpn evpn
    neighbor 192.168.11.2 activate
    neighbor 192.168.11.2 allowas-in             ! eBGP only
    neighbor fd00:11::2 activate
    neighbor fd00:11::2 allowas-in               ! eBGP only
    neighbor 192.168.10.3 activate               ! RR cp-1, no allowas-in
    neighbor fd00:10::3 activate
    neighbor 192.168.10.4 activate               ! RR cp-2, no allowas-in
    neighbor fd00:10::4 activate
    advertise-all-vni
    advertise-svi-ip
  exit-address-family
```
...
- **RR disabled**: without `routeReflector` section in Underlay, system behaves identically to baseline (no RR stanzas in FRR config)
...
**CI lane**: run the full existing e2e suite with route reflection configured on a 4-node cluster. The RR should be transparent — all existing tests must pass without modification.
