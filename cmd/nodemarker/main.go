...
func main() {
	...
	flag.BoolVar(&args.disableCertRotation, "disable-cert-rotation", false, "disable automatic generation and rotation of webhook TLS certificates/keys")
	flag.BoolVar(&args.restartOnRotatorSecretRefresh, "restart-on-rotator-secret-refresh", false, "Restart the pod when the rotator refreshes its cert.")
	flag.StringVar(&args.certDir, "cert-dir", "/tmp/k8s-webhook-server/serving-certs", "The directory where certs are stored")
	flag.StringVar(&args.certServiceName, "cert-service-name", "openpe-webhook-service", "The service name used to generate the TLS cert's hostname")
	...
	if args.enableWebhook && !args.disableCertRotation {
		...
		if err := setupCertRotation(startListeners, mgr, logger, args.namespace, args.certDir, args.certServiceName, args.restartOnRotatorSecretRefresh); err != nil {
