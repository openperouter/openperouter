...
// IsUnderlayVethInDefaultNetns checks whether the interface exists
// in the default network namespace on nodeName.
func IsUnderlayVethInDefaultNetns(nodeName, intf string) bool {
	exec := executor.ForContainer(nodeName)
	_, err := exec.Exec("ip", "link", "show", intf)
	return err == nil
}

// IsUnderlayVethInPerouterNetns checks whether the interface exists
// inside the perouter network namespace on nodeName.
func IsUnderlayVethInPerouterNetns(nodeName, intf string) bool {
	exec := executor.ForContainer(nodeName)
	_, err := exec.Exec("ip", "netns", "exec", namedNetns, "ip", "link", "show", intf)
	return err == nil
}

var addrRegexp = regexp.MustCompile(`\s(inet6?\s+\S+)`)

// UnderlayVethAddresses returns the non-link-local IP addresses assigned to the
// interface in the default netns on nodeName, sorted and newline-joined.
func UnderlayVethAddresses(nodeName, intf string) (string, error) {
	exec := executor.ForContainer(nodeName)
...
// UnderlayVethIsUp checks whether the interface in the default netns
// on nodeName has state UP.
func UnderlayVethIsUp(nodeName, intf string) bool {
	exec := executor.ForContainer(nodeName)
