...
var _ = Describe("SRV6 routes between bgp and the fabric", Ordered, func() {
	...
	BeforeAll(func() {
		...
		Expect(infra.LeafAConfigForSRv6.Reset()).To(Succeed())
		Expect(infra.LeafBConfigForSRv6.Reset()).To(Succeed())
	})
	...
	Context("with L3VPNs", func() {
		...
		It("receives L3VPN routes from the fabric", func() {
			...
			By("announcing L3VPN routes on VNI 100 from leafA")
			Expect(infra.LeafAConfigForSRv6.ChangePrefixes(emptyPrefixes, leafAVRFRedPrefixes, emptyPrefixes)).To(Succeed())
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnRed, Contains, leafAVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnBlue, !Contains, leafAVRFBluePrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnRed, !Contains, leafBVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnBlue, !Contains, leafBVRFBluePrefixes)

			By("announcing L3VPN routes on VNI 200 from leafA")
			Expect(infra.LeafAConfigForSRv6.ChangePrefixes(emptyPrefixes, leafAVRFRedPrefixes, leafAVRFBluePrefixes)).To(Succeed())
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnRed, Contains, leafAVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnBlue, Contains, leafAVRFBluePrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnRed, !Contains, leafBVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnBlue, !Contains, leafBVRFBluePrefixes)

			By("announcing L3VPN routes on both VNIs from leafB")
			Expect(infra.LeafBConfigForSRv6.ChangePrefixes(emptyPrefixes, leafBVRFRedPrefixes, leafBVRFBluePrefixes)).To(Succeed())
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnRed, Contains, leafAVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnBlue, Contains, leafAVRFBluePrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnRed, Contains, leafBVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnBlue, Contains, leafBVRFBluePrefixes)
			...
			Expect(infra.LeafAConfigForSRv6.ChangePrefixes(emptyPrefixes, emptyPrefixes, leafAVRFBluePrefixes)).To(Succeed())
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnRed, !Contains, leafAVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnBlue, Contains, leafAVRFBluePrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnRed, Contains, leafBVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnBlue, Contains, leafBVRFBluePrefixes)
			...
			Expect(infra.LeafAConfigForSRv6.ChangePrefixes(emptyPrefixes, emptyPrefixes, emptyPrefixes)).To(Succeed())
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnRed, !Contains, leafAVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafAConfigForSRv6, l3vpnBlue, !Contains, leafAVRFBluePrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnRed, Contains, leafBVRFRedPrefixes)
			checkRouteFromLeaf(infra.LeafBConfigForSRv6, l3vpnBlue, Contains, leafBVRFBluePrefixes)
	...
	Context("with L3VPNs and frr-k8s", func() {
		...
		It("translates L3VPN incoming routes as BGP routes", func() {
			...
			Expect(infra.LeafAConfigForSRv6.ChangePrefixes(emptyPrefixes, leafAVRFRedPrefixes, emptyPrefixes)).To(Succeed())
			Expect(infra.LeafBConfigForSRv6.ChangePrefixes(emptyPrefixes, leafBVRFRedPrefixes, emptyPrefixes)).To(Succeed())
			...
			for _, frrk8s := range frrk8sPods {
				ginkgo.GinkgoWriter.Printf("checking on FRR-K8S pod %s: for HostSession of L3VPN %s, "+
					"looking for prefixes %v, should exist %t", frrk8s.Name, l3vpnRed.Name, leafAVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafAVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafBVRFRedPrefixes, ShouldExist)
			}
			...
			for _, frrk8s := range frrk8sPods {
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafAVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafBVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnBlue.Spec.HostSession, leafAVRFBluePrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnBlue.Spec.HostSession, leafBVRFBluePrefixes, ShouldExist)
			}
			...
			for _, frrk8s := range frrk8sPods {
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafAVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnRed.Spec.HostSession, leafBVRFRedPrefixes, ShouldExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnBlue.Spec.HostSession, leafAVRFBluePrefixes, ShouldNotExist)
				checkBGPPrefixesForHostSession(frrk8s, *l3vpnBlue.Spec.HostSession, leafBVRFBluePrefixes, ShouldNotExist)
			}
		})
	})
	...
	Context("testing end to end integration between a pod and the blue / red hosts", func() {
		...
		BeforeAll(func() {
			...
			Expect(infra.LeafAConfigForSRv6.RedistributeConnected()).To(Succeed())
			Expect(infra.LeafBConfigForSRv6.RedistributeConnected()).To(Succeed())
		...
		DescribeTable("should be able to reach the hosts from the test pod and vice versa", func(
			...
			// Traffic leaving the OpenPERouter will be SNATted to the host IP address, meaning that leafA/leafB are
			...
			Entry("l3vpn red host A ipv4", l3vpnRed, "hostA_red", infra.HostARedIPv4),
			Entry("l3vpn red host B ipv4", l3vpnRed, "hostB_red", infra.HostBRedIPv4),
			Entry("l3vpn blue host A ipv4", l3vpnBlue, "hostA_blue", infra.HostABlueIPv4),
			Entry("l3vpn blue host B ipv4", l3vpnBlue, "hostB_blue", infra.HostBBlueIPv4),
			Entry("l3vpn red host A ipv6", l3vpnRed, "hostA_red", infra.HostARedIPv6),
			Entry("l3vpn red host B ipv6", l3vpnRed, "hostB_red", infra.HostBRedIPv6),
			Entry("l3vpn blue host A ipv6", l3vpnBlue, "hostA_blue", infra.HostABlueIPv6),
			Entry("l3vpn blue host B ipv6", l3vpnBlue, "hostB_blue", infra.HostBBlueIPv6),
		)
	})
})
...
var _ = Describe("SRV6 routes between bgp and the fabric with iBGP testing e2e integration between a pod and the red hosts", func() {
	...
	BeforeEach(func() {
		...
		Expect(infra.LeafAConfigForSRv6.Configure(infra.LeafConfiguration{
			Red:         infra.Addresses{RedistributeConnected: true},
			Default:     infra.Addresses{RedistributeConnected: true},
			PERouterASN: 64520,
		})).To(Succeed())
		Expect(infra.LeafBConfigForSRv6.Configure(infra.LeafConfiguration{
			Red:         infra.Addresses{RedistributeConnected: true},
			Default:     infra.Addresses{RedistributeConnected: true},
			PERouterASN: 64520,
		})).To(Succeed())
