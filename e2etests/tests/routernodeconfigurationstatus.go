...
// Helper function to convert set to sorted slice for consistent error messages
...
// Helper function to get stable RouterNodeConfigurationStatus list with validation
	...
	Context("Lifecycle Management", func() {
		...
		It("should automatically create RouterNodeConfigurationStatus resources with the expected name for each node with a running controller", func() {
			...
			Eventually(func() error {
				...
				uncoveredNodes := setDifference(controllerNodesSet, resourceNamesSet)     // nodes without resources
				unmatchedResources := setDifference(resourceNamesSet, controllerNodesSet) // resources without controllers
		...
		It("should have proper owner references linking to Node resources", func() {
			// Wait for RouterNodeConfigurationStatus resources to be created and verify owner references
			Eventually(func() error {
				// Get stable status list with validation
				statusList, err := getStableStatusList(k8sClient)
				if err != nil {
		...
		It("should track multiple resource failures and recover properly", func() {
			...
			// Step 5: Reference it from an invalid L2VNI (nonexistent bridge)
