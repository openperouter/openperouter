	...
	ginkgo.Context("Underlay", func() {
		...
		ginkgo.It("moves the underlay interface back to the default netns on deletion", func() {
			...
			for _, node := range nodes {
				Eventually(func() bool {
					return openperouter.IsUnderlayVethInDefaultNetns(node.Name, underlayNIC)
			...
			for _, node := range nodes {
				addrs, err := openperouter.UnderlayVethAddresses(node.Name, underlayNIC)
			...
			for _, node := range nodes {
				Eventually(func() bool {
					return openperouter.IsUnderlayVethInPerouterNetns(node.Name, underlayNIC)
			...
			for _, node := range nodes {
				Expect(openperouter.UnderlayVethIsUp(node.Name, underlayNIC)).To(BeTrueBecause(
					"%s should be UP on %s after underlay deletion", underlayNIC, node.Name))

				addrsAfter, err := openperouter.UnderlayVethAddresses(node.Name, underlayNIC)
