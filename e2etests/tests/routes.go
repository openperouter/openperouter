	...
	FContext("testing e2e integration between a pod and the blue / red hosts", func() {
		...
		BeforeEach(func() {
			...
			advertisePodToVNI := func(pod *corev1.Pod, vni v1alpha1.L3VNI) []frrk8sapi.FRRConfiguration {
				...
				for _, podIP := range pod.Status.PodIPs {
					...
					config, err := frrk8s.ConfigFromVNI(vni, frrk8s.WithNodeSelector(nodeSelector), frrk8s.AdvertisePrefixes(podIP.IP+cidrSuffix))
					Expect(err).NotTo(HaveOccurred())
		...
		DescribeTable("should be able to reach the hosts from the test pod and vice versa", func(
			...
			Eventually(func() error {
				...
				url := fmt.Sprintf("http://%s:8090/clientip", externalHostIP)
				...
				url = fmt.Sprintf("http://%s:8090/hostname", externalHostIP)
				res, err = podExecutor.Exec("curl", "-sS", url)
				if err != nil {
					return fmt.Errorf("curl %s:8090 failed: %s", externalHostIP, res)
				}
				...
				res, err = externalHostExecutor.Exec("curl", "-sS", fmt.Sprintf("http://%s:8090/clientip", podIP))
				if err != nil {
