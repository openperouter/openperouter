...
var _ = Describe("Routes between bgp and the fabric with iBGP", Ordered, func() {
	var cs clientset.Interface
	var routers openperouter.Routers
	...
	Context("testing e2e integration between a pod and the blue / red hosts", func() {
		...
		It("should be able to reach the hosts from the test pod and vice versa vni red hat A ipv4", func() {
			vni := vniRed
			hostName := "hostA_red"
