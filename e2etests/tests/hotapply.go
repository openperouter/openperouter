...
var _ = Describe("Hot-Apply Configuration Changes", Ordered, func() {
	...
	It("adds a neighbor without restarting the router pods", func() {
		...
		Consistently(func() error {
			...
			// DaemonsetRolled returns nil when pods HAVE been rolled.
			// We expect it to return an error (pods NOT rolled = same names).
			if err := openperouter.DaemonsetRolled(initialRouters, currentRouters); err == nil {
				return fmt.Errorf("router pods were unexpectedly restarted after adding a neighbor")
			}
			return nil
