...
var _ = Describe("Mixed L3VPN and L3VNI coexistence in different VRFs", Ordered, func() {
	...
	// with SRv6 H.Encaps.Red and only an L3VPN but no L2VNI as packets leave via eth0 which is managed by the default
	// CNI plugin and which always has an MTU of 1500.
	DescribeTable("should allow packets with maximum MTU in different VRFs", func(
		...
		By("getting the pod executor for first pod connected to L3VPN")
		firstPodExecutor := executor.ForPod(firstPod.Namespace, firstPod.Name, "agnhost")

		By("getting the pod executor for third pod connected to L3VNI")
		thirdPodExecutor := executor.ForPod(thirdPod.Namespace, thirdPod.Name, "agnhost")

		By(fmt.Sprintf("checking MTU correctly configured on first pod - expecting overhead %d", expectedL3VPNOverhead))
		checkMaximumMTU(firstPodExecutor, "net1", baseMTU-expectedL3VPNOverhead)

		By("checking ping with maximum MTU works from first pod to second pod")
		pingWithMaximumMTU(firstPodExecutor, discardAddressLength(secondPodIP), baseMTU-expectedL3VPNOverhead)

		By("checking ping with maximum MTU works from first pod via L3VPN to hostSRV6Red")
		pingWithMaximumMTU(firstPodExecutor, infra.HostSRV6RedIPv4, baseMTU-expectedL3VPNOverhead)

		By(fmt.Sprintf("checking MTU correctly configured on third pod - expecting overhead %d", expectedL3VNIOverhead))
		checkMaximumMTU(thirdPodExecutor, "net1", baseMTU-expectedL3VNIOverhead)

		By("checking ping with maximum MTU works from third pod to fourth pod")
		pingWithMaximumMTU(thirdPodExecutor, discardAddressLength(fourthPodIP), baseMTU-expectedL3VNIOverhead)

		By("checking ping with maximum MTU works from third pod via L3VNI to hostABlue")
		pingWithMaximumMTU(thirdPodExecutor, infra.HostABlueIPv4, baseMTU-expectedL3VNIOverhead)
	},
