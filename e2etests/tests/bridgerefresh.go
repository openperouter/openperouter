...
var _ = Describe("BridgeRefresher E2E - Type 2 Route Persistence", Ordered, func() {
	...
	Context("Type 2 Route Persistence with Silent Workload", func() {
		...
		BeforeEach(func() {
			...
			silentPod, err = k8s.CreateSilentPod(cs, "silent-pod", testNamespace,
		...
		It("should maintain Type 2 MAC+IP route for silent workload", func() {
			// Get the VTEP IP for the node where the pod is running
			vtepIP, err := openperouter.VtepIPForNode(infra.Underlay.Spec.EVPN.VTEPCIDR, podNode)
			Expect(err).NotTo(HaveOccurred())
			// Strip the /32 suffix from VTEP IP
			vtepIPOnly := stripCIDRMask(vtepIP)

			// The silent pod IP without mask
			podIPOnly := stripCIDRMask(silentPodIP)
			...
			// This is the critical test: without BridgeRefresher, the neighbor entry
			// would go STALE -> DELETE after gc_stale_time, causing the Type 2 route
			...
			Consistently(func() error {
				for exec := range routers.GetExecutors() {
					...
					if !evpn.ContainsType2MACIPRouteForVNI(podIPOnly, vtepIPOnly, l2VNI) {
						return fmt.Errorf("Type 2 MAC+IP route for %s was WITHDRAWN from router %s - BridgeRefresher may not be working", podIPOnly, exec.Name())
					}
				}
				return nil
