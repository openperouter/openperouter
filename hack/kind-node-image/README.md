...
This directory contains the build infrastructure for a custom kind node image with OpenVSwitch (OVS) pre-installed.
This eliminates the need to install OVS at runtime and improves reliability in development and CI/CD environments.
...
```bash
export NODE_IMAGE=quay.io/openperouter/kind-node-ovs:v1.32.2
make deploy
```
...
1. Check that the base kind image exists: `docker pull kindest/node:v1.32.2`
